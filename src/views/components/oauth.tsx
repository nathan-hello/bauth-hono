export function OauthButtons({ callbackURL }: { callbackURL?: string }) {
    return (
        <div class="flex flex-col w-full gap-3 justify-center items-center ">
            ...
            <AppleButton callbackURL={callbackURL} />
        </div>
    );
}
...
function GoogleButton({ callbackURL }: { callbackURL?: string }) {
    return (
        <form method="post" action={routes.auth.oauthGoogle}>
            <input type="hidden" name="callbackURL" value={callbackURL ?? routes.auth.dashboard} />
            <button type="submit" class="gsi-material-button" style="width:400">
...
function AppleButton({ callbackURL }: { callbackURL?: string }) {
    return (
        <form method="post" action={routes.auth.oauthApple} class="cursor-pointer select-none">
            <input type="hidden" name="callbackURL" value={callbackURL ?? routes.auth.dashboard} />
