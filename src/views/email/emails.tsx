...
export function EmailOtp({ email, otp, url }: { email: string; otp: string; url: string }) {
  return (
    <EmailLayout url={url}>
      <p style={{ fontSize: "1rem", margin: 0 }}>{copy.email_otp_body} <strong>{email}</strong>:</p>
...
export function Email2fa({ email, otp, url }: { email: string; otp: string; url: string }) {
  return (
    <EmailLayout url={url}>
      <p style={{ fontSize: "1rem", margin: 0 }}>{copy.email_2fa_body} <strong>{email}</strong>:</p>
      <p style={{ fontSize: "2rem", fontWeight: "bold", letterSpacing: "0.15em", margin: "16px 0" }}>{otp}</p>
  ...
  return (
    <EmailLayout url={url}>
      <p style={{ fontSize: "15px", margin: "0 0 16px" }}>
        {copy.email_change_body}
      </p>
      <p>
        {copy.email_change_from} <strong>{oldEmail}</strong>{" "}
        {copy.email_change_to} <strong>{newEmail}</strong>
      </p>
